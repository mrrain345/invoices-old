...
function process0999(digits: number[]) {
  let result = "";
...
function classify(digits: number[]) {
  if (digits.join("") === "001") {
    return "one";
  } else if (
    digits[1] !== 1 &&
    (digits[2] === 2 || digits[2] === 3 || digits[2] === 4)
  ) {
    return "few";
  } else {
    return "many";
...
export default function polishToWords(number: number): string {
  const digits: number[] = [];
  let result = "";

  const digitsStr = String(Math.floor(Math.abs(number))).split("");

  for (let i = 0; i < digitsStr.length; i++) {
    digits[i] = parseInt(digitsStr[i], 10);
  }
